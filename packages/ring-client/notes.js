...
export function deriveNullifier({ noteSecret, pool }) {
  ...
  assertHex(pool, 'pool');
  return `0x${createHash('sha256').update(`PROJECT_X_NULLIFIER_V1|${noteSecret.toLowerCase()}|${pool.toLowerCase()}`).digest('hex')}`;
}
...
export function derivePaymentContext({ pool, chainId, recipient, denomination }) {
  ...
  return `0x${createHash('sha256').update(`PROJECT_X_PAYMENT_V1|${pool.toLowerCase()}|${chainId}|${recipient.toLowerCase()}|${denomination}`).digest('hex')}`;
}
