...
| PQ account | **Mock chain.** Real FORS keys, simulated registry — Manan's live adapter is in progress |
| Ring source | **Real deposits**, read from the pool's events — the subgraph is not deployed yet |
...
Both spend a real seeded note and burn one attester index. They are scripts you
run on purpose, not tests that run themselves.

## Two budgets to watch

- **The attester's FORS key has 32 signatures.** Each settled payment uses one;
  two are used as of the first browser settlement. The registry refuses the
  33rd rather than letting forgery odds climb, so rotate before then.
